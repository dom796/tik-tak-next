...
export function GameStatus({ game }: { game: GameDomain.GameEntity }) {
  ...
  switch (game.status) {
    ...
    case "gameOverDraw":
      return <div className="text-lg font-medium text-muted-foreground rounded-md bg-muted/50 px-3 py-2 text-center flex items-center justify-center gap-2"><span>🤝</span>{t("draw")}</div>;
  }
}